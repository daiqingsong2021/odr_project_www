...
import {  Modal, Form, Table, Input, Button, Select, Row, Col, DatePicker,Descriptions } from 'antd';
...
import { findAcmlog,dailyChangeVersionList } from "@/api/api"
const { Item } = Form;
...
export class AddModal extends Component {
    ...
    getList=()=>{
        ...
        // let url = `?recordId=${id}`
        let url = `?moudleRecordId=${id}`
        axios.get(dailyChangeVersionList(this.state.pageSize, this.state.currentPageNum) + url).then(res => {
            this.setState({
                data: res.data.data,
            })
        })
    ...
    render() {
        ...
        const columnsNew = [
            ...
            },
            {
                ...
                render:(text,record) => {
                    ...
                    return <div dangerouslySetInnerHTML={{__html: textNew}} />;
                }
            },
            {
                title: '修改备注',
        ...
        const statusVo =record&& record.statusVo ? record.statusVo : record.reviewStatusVo
        return (
                    <Modal className={style.main}
                        ...
                        centered={true} title={statusVo.code =='INIT'?'修改日志':'调整日志'} visible={this.props.modalVisible}
                            ...
                            <div className={style.tipBox}>
                                <div className={style.topTags}>
                                        ...
                                        <Col span={8}>
                                            <Item label='状态' {...formItemLayout}>
                                                {getFieldDecorator('status', {
                                                    initialValue: record ? statusVo.code : null,
                                                    rules: [
                                                        {
                                                            required: true,
                                                            message: '请选择状态',
                                    ...
                                    <Descriptions column={4}>
                                        <Descriptions.Item label="日期">{record && record.recordTime && record.recordTime.substring(0, 10) || record && record.recordDay || ''}</Descriptions.Item>
                                        <Descriptions.Item label="线路">{record && record.lineName || ''}</Descriptions.Item>
                                    </Descriptions>
                                </div>
                            </div>
                        </div>
