...
import { Tooltip, Icon, Spin, notification,Tabs } from 'antd';
import dynamic from 'next/dynamic';
import { connect } from 'react-redux';
import style from './style.less';
import axios from '@/api/axios'
import * as layoutUtil from '@/components/public/Layout/layoutUtil'
const { TabPane } = Tabs;
class TopTags extends React.Component {
	constructor(props) {
      ...
      rightMenu : {"1":[]},
			...
			defaultActiveKey:'0',//tabs页签默认显示第一个
		};
	}
			...
			if (res.data.data) {
				this.setState({
					...
					if(this.props.openRight){
						...
						this.openRightBox('Suzhou/Modules/Security/InsidelTraining/Study', 11,'学习人员')
					}else if(this.state.rightMenu && groupCode != -1 && this.state.rightMenu[groupCode]){
						this.setState({defaultActiveKey:'0'});
						this.openRightBox(this.state.rightMenu[groupCode][0].url, 0,this.state.rightMenu[groupCode][0].menuName);
					}
				})
			}
		})
  ...
  getLabelInfo=()=>{
	...
	componentWillReceiveProps(newProps, state) {
		if(newProps.rightTagShow){
			this.getTagsList();
			this.props.handleDouble(this,false);
		}
	...
	closeRightBox = () => {
		...
		this.setState({
		  ...
		  // 主页面设置为初始化宽度
	  this.renderInitWidth();
	  if(this.props.closeInfo){
        console.log(11,this.props.closeInfo)
   ...
   */
    ...
    const width = widowsWidth; // 右侧弹出框宽度;
	...
	test =(key,v)=>{
		let groupCode = this.props.groupCode != undefined ? this.props.groupCode : 1;
		if(this.state.rightMenu && groupCode != -1 && this.state.rightMenu[groupCode]){
			this.openRightBox(this.state.rightMenu[groupCode][key].url, key+1,this.state.rightMenu[groupCode][key].menuName)
		}
	}
	render() {
		const widowsWidth = document.documentElement.clientWidth || document.body.clientWidth;
		...
		const width = lastRenderWidth ? widowsWidth: (widowsWidth); // 右侧弹出框宽度;
	...
	let labelMainId = this.props.menuCode + "-LabelsGroup";
		return (
			<div>
				{/* <div className={style.wrap} style={{width:'100%',height:parseInt(this.props.height)+100+"px",position: 'fixed',marginTop:'-48px'}}></div> */}
			<div className={style.main} id = {labelMainId} style={{top:this.props.flowRight?'40px':'0'}}>
				{this.state.content && (
					<div id={id} className={style.content} data-content={"LabelsGroup"} style={{ width: '100%' ,height: this.props.height ? parseInt(this.props.height) +10 +'px' : this.state.height+190}}>
						...
						<Tabs defaultActiveKey={this.state.defaultActiveKey} onTabClick={this.test.bind(this)}>
									{this.state.rightMenu[groupCode].length > 0&&this.state.rightMenu[groupCode].map((item,index)=>{
										return(
										<TabPane tab={item.menuName} key={index}>
											{this.state.isLoading ? (
												<Spin />
											) : (
												<ComponentsForm
