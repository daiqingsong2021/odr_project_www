...
class AddTimeTable1 extends Component {
  constructor(props) {
    ...
    this.state = {
      //当日计划
      dataSource: [{
        id: 1, lineName: '当日计划', trainListNumber: '', passengerMiles: '', emptyMiles: '', 
      }],
      //上行线路
      dataTop1: [{
      ...
      dataTop2: [{
        id: 3, lineType: 0, typeCode: 'top2', edit: true, rowType: 'title', typeName: '上行完整载客前空驶', period: '', startStation: '',
        endStation: '', inStation: '', coloumns: 0, remark: '', mileage: 0, newLineStationMileage: 0, oldLineStationMileage: 0
      }],
      dataTop3: [{
        id: 4, lineType: 0, typeCode: 'top3', edit: true, rowType: 'title', typeName: '上行完整空驶', period: '', startStation: '',
        endStation: '', inStation: '', coloumns: 0, remark: '', mileage: 0, newLineStationMileage: 0, oldLineStationMileage: 0
      }],
      dataTop4: [{
        id: 5, lineType: 0, typeCode: 'top4', edit: true, rowType: 'title', typeName: '上行回段完整空驶', period: '', startStation: '',
        endStation: '', inStation: '', coloumns: 0, remark: '', mileage: 0, newLineStationMileage: 0, oldLineStationMileage: 0
      }],
      //下行线路
      dataDown1: [{
        id: 6, lineType: 1, typeCode: 'down1', edit: false, rowType: 'title', typeName: '下行完整载客', period: '', startStation: '',
        endStation: '', inStation: '', coloumns: 0, remark: '', mileage: 0, newLineStationMileage: 0, oldLineStationMileage: 0
      }],
      dataDown2: [{
        id: 7, lineType: 1, typeCode: 'down2', edit: true, rowType: 'title', typeName: '下行完整载客后空驶', period: '', startStation: '',
        endStation: '', inStation: '', coloumns: 0, remark: '', mileage: 0, newLineStationMileage: 0, oldLineStationMileage: 0
      }],
      dataDown3: [{
        id: 8, lineType: 1, typeCode: 'down3', edit: true, rowType: 'title', typeName: '下行完整空驶', period: '', startStation: '',
        endStation: '', inStation: '', coloumns: 0, remark: '', mileage: 0, newLineStationMileage: 0, oldLineStationMileage: 0
      }],
      dataDown4: [{
        id: 9, lineType: 1, typeCode: 'down4', edit: true, rowType: 'title', typeName: '下行回段空驶', period: '', startStation: '',
        endStation: '', inStation: '', coloumns: 0, remark: '', mileage: 0, newLineStationMileage: 0, oldLineStationMileage: 0
      }],
      dataDown5: [{
        id: 10, lineType: 1, typeCode: 'down5', edit: true, rowType: 'title', typeName: '下行出段空驶', period: '', startStation: '',
        endStation: '', inStation: '', coloumns: 0, remark: '', mileage: 0, newLineStationMileage: 0, oldLineStationMileage: 0
      }],
      lineArr: [],  //线路表
      startStation: [], //始发站
      ...
      idTemp: 13,
      supStation: [],  //辅助站点
      disChuduan: ['top2','top3', 'top4', 'down3', 'down4', 'down5'], //出段需要录入的类型
      disRuduan: ['top2','top3', 'top4','down2','down3', 'down4', 'down5'],//入段需要录入的类型
      isOk:false, //保存按钮是否置灰
    }
  }
      ...
      this.setState(
        {
          ...
          this.getStationFun(this.state.line, '3', '', '', (res) => { //获取全部辅助线
            this.setState({
              supStation: res
            })
          })
  ...
  deleteFun = (record) => {
    switch (record.typeCode) {
      case 'top2':
        let { dataTop2 } = this.state 
        dataTop2.splice(dataTop2.findIndex(item => record.id == item.id), 1)
        this.setState({ dataTop2 })
        break
        case 'top3':
        let { dataTop3 } = this.state
        dataTop3.splice(dataTop3.findIndex(item => record.id == item.id), 1)
        this.setState({ dataTop3 })
        break
        case 'top4':
        let { dataTop4 } = this.state
        dataTop4.splice(dataTop4.findIndex(item => record.id == item.id), 1)
        this.setState({ dataTop4 })
        break
      case 'down2':
        let { dataDown2 } = this.state
        dataDown2.splice(dataDown2.findIndex(item => record.id == item.id), 1)
        this.setState({ dataDown2 })
        break
      case 'down3':
        let { dataDown3 } = this.state
        dataDown3.splice(dataDown3.findIndex(item => record.id == item.id), 1)
        this.setState({ dataDown3 })
        break
      case 'down4':
        let { dataDown4 } = this.state
        dataDown4.splice(dataDown4.findIndex(item => record.id == item.id), 1)
  ...
  //设置当日计划
  setTodayPlanFun = () => {
    const { dataSource, dataTop1, dataTop2, dataTop3, dataTop4, dataDown1, dataDown2, dataDown3, dataDown4, dataDown5 } = this.state
    let trainListNumber = 0,
      passengerMiles = 0,
      emptyMiles = 0
    dataTop1.map(item => { passengerMiles += Number(item.mileage) ;  })
    dataTop2.map(item => { trainListNumber += Number(item.coloumns);  emptyMiles += Number(item.mileage) ;})
    dataTop3.map(item => { trainListNumber += Number(item.coloumns); emptyMiles += Number(item.mileage) ;  })
    dataTop4.map(item => { trainListNumber += Number(item.coloumns); emptyMiles += Number(item.mileage) ;  })
    dataDown1.map(item => { trainListNumber += Number(item.coloumns); emptyMiles += Number(item.mileage) ; })
    dataDown2.map(item => { trainListNumber += Number(item.coloumns); emptyMiles += Number(item.mileage) ;  })
    dataDown3.map(item => { trainListNumber += Number(item.coloumns); emptyMiles += Number(item.mileage) ;  })
    dataDown4.map(item => { trainListNumber += Number(item.coloumns); emptyMiles += Number(item.mileage) ;  })
    dataDown5.map(item => {  passengerMiles += Number(item.mileage) ;  })
    this.setState({
      dataSource: [{
        ...dataSource[0],
        trainListNumber,
  ...
  selectLines = (value, record, dataType) => {
    ...
    switch (record.typeCode) {
      case 'top1':
        ...
        this.setState({ dataTop1 }, () => { this.setTodayPlanFun() })
        break
      case 'top2':
        let { dataTop2 } = this.state
        ...
        this.setState({ dataTop2 }, () => { this.setTodayPlanFun() })
        break
      case 'top3':
        ...
        if (dataType == 'Chuduan') {
          ...
          this.getMileageFun(record.period, record.startStation, record.endStation, record.inStation, '0', (data) => {
            dataTop3[indexTop3].mileage = (data.stationMileage * value).toFixed(3)
      ...
      case 'top4':
        ...
        if (dataType == 'Chuduan') {
          ...
          this.getMileageFun(record.period, record.startStation, record.endStation, record.inStation, '0', (data) => {
            dataTop4[indexTop4].mileage = (data.stationMileage * value).toFixed(3)
      ...
      case 'down1':
        ...
        this.setState({ dataDown1 }, () => { this.setTodayPlanFun() })
        break
      case 'down2':
        let { dataDown2 } = this.state
        ...
        this.setState({ dataDown2 }, () => { this.setTodayPlanFun() })
        break
      case 'down3':
        ...
        if (dataType == 'Chuduan') {
          ...
          this.getMileageFun(record.period, record.startStation, record.endStation, record.inStation, '1', (data) => {
            dataDown3[indexDown3].mileage = (data.stationMileage * value).toFixed(3)
      ...
      case 'down4':
        ...
        if (dataType == 'Chuduan') {
          ...
          this.getMileageFun(record.period, record.startStation, record.endStation, record.inStation, '1', (data) => {
            dataDown4[indexDown4].mileage = (data.stationMileage * value).toFixed(3)
  ...
  checkChuduan = (data, record) => {
    if (data.value&&record.period) {
      this.getStationFun(this.state.line, '3', record.period, '', (res) => {
  ...
  checkRuduan = (data, record) => {
    if (data.value&&record.endStation) {
  ...
  handleOk = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (err) {
        ...
        let { dataSource, dataTop1, dataTop2, dataTop3, dataTop4,
          dataDown1, dataDown2, dataDown3, dataDown4, dataDown5 } = this.state
        let trainScheduleUpStationForm = [...dataTop1, ...dataTop2, ...dataTop3, ...dataTop4]
        let trainScheduleDownStationForm = [...dataDown1, ...dataDown2, ...dataDown3, ...dataDown4, ...dataDown5]
        const data = {
          line: values.line,
    ...
    const columns1 = [
      {
        ...
        dataIndex: 'typeName',
        render: (text, record) => {
          return <span>{text ? text : ''}{(record.typeCode == 'down4') &&
            record.rowType == 'title' && <Tooltip placement="top"
              title={'按下行方向，车辆驶回停车场/车辆段'}>
      ...
      },
      {
        ...
        render: (text, record) => {
          return <span>{this.state.disChuduan.indexOf(record.typeCode) > -1 ? <Item style={{ margin: 0 }}>
            ...
            })(<Select showSearch placeholder="请选择" disabled={false} allowClear dropdownMatchSelectWidth={false}
              style={{ width: 150 }}
