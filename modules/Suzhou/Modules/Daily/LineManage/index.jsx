...
import { getLineFoundationList } from '@/modules/Suzhou/api/suzhou-api';
import axios from '@/api/axios';
import TopTags from './TopTags/index';
import notificationFun from '@/utils/notificationTip';
//二级页面
import LineManageInfo from './Info/index'
// 布局
...
class LineManage extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            activeIndex: null,
            status: '', //状态
            LineManageInfoShow: false,   //show详情页
            record: {},
            startTime: '',  //查询开始日期
            endTime: '', //查询结束日期
            line: '',    //线路
    ...
    */
    ...
    //获取主列表数据
    getList = (callBack) => {
        axios.get(getLineFoundationList).then(res => {
    ...
    openInfo = (record) => {
        this.setState({
            record,
            LineManageInfoShow: true
        })
    }
    //关闭详情页面
    handleCancel = () => {
        this.setState({
            LineManageInfoShow: false
        })
    }
    //搜索
    ...
    render() {
        const columns = [
            ...
            {
                title: '安全运行起始日期',
                dataIndex: 'saftOperationTime',
                key: 'saftOperationTime',
                render: (text, record) => {
                    return <span >{text ? text : ''}</span>
                }
            },
            ...
            },
            {
                ...
                key: 'x',
                render: (text, record) => {
                    return <span onClick={this.changeRight.bind(this, record)} style={{ color: 'rgb(45,182,244)', cursor: 'pointer' }}>进入</span>
                }
            },
        ];
