...
class Driving extends Component {
    ...
    */
    ...
    getList = (currentPageNum, pageSize, callBack) => {
        ...
        axios.get(queryDailyReportList(pageSize, currentPageNum)+`?line=${line}&startTime=${searcher.startTime}&endTime=${searcher.endTime}&reportType=0&reviewStatus=${searcher.reviewStatus}`).then(res => {
                ...
                render:(text,record)=>{
                    return(
                        <div>
                            ...
                            {this.state.permission && this.state.permission.indexOf('LIST_DOWNLOAD') !== -1 && <Button size='small' type="primary" onClick={this.downloadFile.bind(this,record.loadId)}>下载</Button>}
                        </div>
                    )
                }
            }
